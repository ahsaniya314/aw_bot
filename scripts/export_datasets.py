...
from analyze_dataset import load_normalization_dict, load_nlp_dataset, normalize_text
...
def export_dataset_sebelum_normalisasi(dataset):
    ...
    with open(output_file, mode="w", encoding="utf-8", newline="") as file:
        ...
        for row in dataset:
            entities_str = " | ".join([f"{k}: {v}" for k, v in row["entities"].items()]) if row["entities"] else ""
...
def export_dataset_intent(norm_dict):
    ...
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump({
            "total_intent": len(intent_patterns),
