...
for line in main_lines:
    ...
    if indent == 4 and line.lstrip().startswith('if call.data') or line.lstrip().startswith('if chat_id not in ctx.user_sessions'):
        if current_block:
...
for cond, block in blocks:
    if "mb_" in cond or "mm_" in cond or "btn_batal_edit" in cond or "pick_del" in cond or "pick_metode" in cond:
        pg_blocks.append("".join(block))
    elif "dashboard" in cond:
        dash_blocks.append("".join(block))
    elif "cmd =" in cond or "chat_id =" in cond or "msg_idx =" in cond or "chat_id not in ctx" in cond:
...
with open('handlers/callback_transaksi.py', 'w', encoding='utf-8') as f:
    ...
    for b in common_blocks: f.write(b)
    for b in trx_blocks: f.write(b)

with open('handlers/callback_pengaturan.py', 'w', encoding='utf-8') as f:
    f.write(prelude)
    f.write("def handle_pengaturan_callbacks(call):\n")
    for b in common_blocks: f.write(b)
    for b in pg_blocks: f.write(b)

print(f"Total blocks: {len(blocks)}, Trx: {len(trx_blocks)}, Pg: {len(pg_blocks)}, Dash: {len(dash_blocks)}, Common: {len(common_blocks)}")
