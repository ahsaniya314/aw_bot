...
class OCRPreprocessor:
    ...
    def resize_image(self, image: np.ndarray, target_size: Tuple[int, int] = None, 
                     scale_factor: float = None) -> Tuple[np.ndarray, Dict]:
        """
    ...
    def pad_image(self, image: np.ndarray, padding_size: int = 50, 
                  padding_color: Tuple[int, int, int] = (255, 255, 255)) -> Tuple[np.ndarray, Dict]:
        """
    ...
    def convert_and_normalize_color(self, image: np.ndarray) -> Tuple[np.ndarray, Dict]:
        ...
        """
        metadata = {
            "method": "color_conversion_normalization"
        }
    ...
    def preprocess_with_proof(self, image_path: str, base_filename: str = "preprocessing") -> Dict:
        ...
        logger.info(f"[OCR Preprocessor] Preprocessing selesai. Bukti disimpan di: {self.output_dir}")
        
        return proof_data


    def visualize_ocr_results(self, image: np.ndarray, ocr_results: List, 
                              output_path: str, draw_text: bool = True, 
                              draw_confidence: bool = True) -> Dict:
        """
        ...
        legend_y = 30
        cv2.putText(vis_image, f"Total: {metadata['total_text_boxes']} boxes", 
                    (10, legend_y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
        legend_y += 30
        if count > 0:
            cv2.putText(vis_image, f"Avg Conf: {metadata['average_confidence']:.2f}", 
                        (10, legend_y), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
