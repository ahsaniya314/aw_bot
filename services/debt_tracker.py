...
from core.master_data import parse_rupiah, format_rupiah
...
def cari_hutang_aktif(db_transaksi, nama_target, cocokkan_nama_fn):
    ...
    for row in semua_data:
        ...
        logger.info(f"[DEBUG] Checking transaction: id={row.get('id')}, nama_pelanggan={repr(nama_db)}, nama_target={repr(nama_target)}")
        ...
        logger.info(f"[DEBUG] Transaction: id={row.get('id')}, status={status}, total={total}, uang_masuk={uang_masuk}, tagihan={tagihan}")
...
# =====================================================
def proses_bayar_tambahan(db_transaksi, db_histori,
                          row_idx, nominal_bayar,
