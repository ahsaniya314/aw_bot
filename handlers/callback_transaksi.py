...
def handle_transaksi_callbacks(call):
    ...
    if cmd in {"ocr_apply_cicilan", "ocr_cancel_cicilan"}:
        ...
        hasil_lines.append("━━━━━━━━━━━━━━━━━━━━━━")
        hasil_lines.append(
            f"💰 <b>Total Cicilan Terpakai:</b> <code>{format_rupiah(total_applied)}</code>"
        )
        if gagal:
    ...
    sessionless_callbacks = [
        ...
        "quick_search_barang",
        "quick_search_status",
        "quick_search_status_lunas",
        "quick_search_status_hutang",
    ]
    ...
    if cmd in sessionless_callbacks or cmd.startswith("guide_"):
        ...
        if cmd == "quick_input_ocr":
            ...
            safe_edit_message(ctx.bot, teks, chat_id, msg_idx, parse_mode="HTML")
...
"""
            ctx.bot.send_message(
                chat_id, teks, parse_mode="HTML", reply_markup=build_main_menu_markup()
            )
            return
    ...
    if chat_id not in ctx.user_sessions:
        ctx.bot.answer_callback_query(
            call.id,
            "❌ Sesi kedaluwarsa atau bot telah di-restart.\nSilakan ketik ulang perintah Anda.",
            show_alert=True,
        )
        return
    ...
    if cmd.startswith(
        ("pick_brg_", "multi_pick_brg_", "pick_edit_row_", "pick_edit_row_full_", "pick_del_row_")
    ):
        ...
        safe_debug_event(
            {
                ...
                "data": {
                    ...
                    "multi_count": len(sess.get("multi_results") or []) if sess else 0,
                },
            }
        )
    ...
    if cmd == "btn_update_kirim":
        ...
        if not row_idx:
            safe_edit_message(
                ctx.bot, "❌ Data tidak ditemukan. Silakan mulai edit kembali.", chat_id, msg_idx
            )
            if chat_id in ctx.user_sessions:
                del ctx.user_sessions[chat_id]
            return
            ...
            if update_mode == "tambah_cicilan" and update_info.get("nominal_baru") is not None:
                ...
                if update_info.get("nominal_baru") is not None:
                    if update_mode == "koreksi_data":
                        ...
                        uang_masuk = (
                            update_info.get("uang_masuk_lama", 0) + update_info["nominal_baru"]
                        )
    ...
    if cmd == "btn_batal_edit":
        safe_answer_callback_query(ctx.bot, call)
        safe_edit_message(ctx.bot, "✅ Edit dibatalkan. Kembali ke menu utama.", chat_id, msg_idx)
        if chat_id in ctx.user_sessions:
            del ctx.user_sessions[chat_id]
        return
    ...
    if cmd.startswith("pel_pilih_"):
        ...
        try:
            ...
            _tampilkan_konfirmasi_pelunasan(
                chat_id, msg_idx, info_hutang, sess.get("nominal_bayar")
            )
        except Exception as e:
    ...
    if cmd == "pel_konfirm_ya":
        ...
        try:
            ...
            if result.get("sukses"):
                ...
                safe_edit_message(
                    ctx.bot,
                    f"❌ Gagal mencatat pelunasan: {result.get('error', 'Unknown error')}",
                    chat_id,
                    msg_idx,
                )
        except Exception as e:
