...
from utils.security import safe_edit_message, safe_debug_event
...
from services.ui_transaksi import kirim_halaman_read, susun_balasan_resume, susun_balasan_update, susun_balasan_multi_resume, tampilkan_menu_kriteria_edit, apply_batch_financials
from services.ui_pengaturan import tampilkan_pilihan_barang
from core.master_data import (
    format_rupiah, parse_rupiah, normalisasi_tanggal_gs,
    get_all_barang, cari_harga_default
)
from services.debt_tracker import hitung_sisa_tagihan, cari_hutang_aktif, proses_bayar_tambahan
...
def tangani_read_data(chat_id, message_id_target):
    if chat_id not in ctx.user_sessions: return
    ...
    if not (f_nama or f_tanggal or f_status or f_barang or f_semua or f_harga or f_metode or konteks_agregasi):
        safe_edit_message(ctx.bot, "🔍 Mohon lebih spesifik, siapakah atau kapan pesanan yang Anda cari? (Contoh: <i>Cek pesanan pak andi</i> atau <i>tampilkan semua transaksi</i>) ", chat_id=chat_id, message_id=message_id_target, parse_mode="HTML")
        del ctx.user_sessions[chat_id]
        return
        
    if not ctx.IS_DB_CONNECTED:
        safe_edit_message(ctx.bot, "❌ Gagal Mengakses: Mode Offline aktif.", chat_id=chat_id, message_id=message_id_target)
        del ctx.user_sessions[chat_id]
        return
        
    safe_edit_message(ctx.bot, f"🔍 Mencari data di Database dengan parameter (Nama: {f_nama or '-'}, Tgl: {f_tanggal or '-'}, Status: {f_status or '-'}, Brg: {f_barang or '-'}, Harga: {f_harga or '-'}, Metode: {f_metode or '-'}) ...", chat_id=chat_id, message_id=message_id_target)
    ...
    try:
        ...
        safe_edit_message(ctx.bot, f"❌ Gagal memindai database: {e}", chat_id=chat_id, message_id=message_id_target)
        del ctx.user_sessions[chat_id]
        return
        
    if len(match_list) == 0:
        debug_info = f"\n\n<i>(Debug: Tgl={f_tanggal or 'Semua'}, Status={f_status or 'Semua'}, Nama={f_nama or 'Semua'})</i>"
        if date_exact and f_tanggal and len(str(f_tanggal)) >= 10 and str(f_tanggal)[2] == "-" and str(f_tanggal)[5] == "-":
            ddmm = str(f_tanggal)[:5]
            by_date = {}
            ...
            if by_date:
                ...
                markup.add(InlineKeyboardButton(f"Tampilkan {rekom_date} ({rekom_count} data)", callback_data=f"read_date_{rekom_date}"))
                markup.add(InlineKeyboardButton("❌ Tutup", callback_data="btn_buang"))
                safe_edit_message(ctx.bot, teks, chat_id=chat_id, message_id=message_id_target, parse_mode="HTML", reply_markup=markup)
                del ctx.user_sessions[chat_id]
                return

        safe_edit_message(ctx.bot, f"📭 <b>Data tidak ditemukan.</b>\nPastikan kriteria pencarian Anda sudah benar.{debug_info}", chat_id=chat_id, message_id=message_id_target, parse_mode="HTML")
        del ctx.user_sessions[chat_id]
        return
    ...
    if konteks_agregasi:
        ...
        top_name = max(pelanggan_stats, key=lambda k: pelanggan_stats[k]['total']) if pelanggan_stats else None
        ...
        if f_nama and konteks_agregasi in ["Total Tunggakan", "Total Transaksi"] and not f_semua:
            ...
            if stats:
                ringkasan_teks += f"⚠️ <b>Total Hutang:</b> <code>{format_rupiah(stats['tagihan'])}</code>\n"
                ringkasan_teks += f"💸 <b>Sudah Dibayar:</b> <code>{format_rupiah(stats['total'] - stats['tagihan'])}</code>\n"
                ringkasan_teks += f"📦 <b>Total Pesanan:</b> {stats['item']} Item ({stats['count']} Nota)\n"
                ringkasan_teks += "━━━━━━━━━━━━━━━━━━━━━━\n"
            ...
            if f_tanggal or f_semua or f_status or f_barang or f_metode or (konteks_agregasi and konteks_agregasi != "Tunggakan Terbanyak"):
                ringkasan_teks += f"📅 <b>LAPORAN ({f_tanggal or 'Hari Ini'})</b>\n"
                ringkasan_teks += f"💰 <b>Omzet Kotor:</b> <code>{format_rupiah(total_omzet)}</code>\n"
                ringkasan_teks += f"💸 <b>Uang Masuk:</b> <code>{format_rupiah(total_uang_msk)}</code>\n"
                ringkasan_teks += f"⚠️ <b>Sisa Tagihan:</b> <code>{format_rupiah(total_tagihan)}</code>\n"
                ringkasan_teks += "━━━━━━━━━━━━━━━━━━━━━━\n"
                ringkasan_teks += "📋 <b>BREAKDOWN STATUS</b>\n"
                for s, val in status_stats.items():
                    ringkasan_teks += f"• {s}: {val['count']} Trx (<code>{format_rupiah(val['total'])}</code>)\n"
                ...
                ringkasan_teks += f"💰 <b>Total Uang:</b> <code>{format_rupiah(total_omzet)}</code>\n"
        ...
        if konteks_agregasi in ["Total Tunggakan", "Tunggakan Terbanyak"]:
            ...
            ringkasan_teks += f"⚠️ <b>Total Sisa Tagihan:</b> <code>{format_rupiah(total_tagihan)}</code>\n"
            ringkasan_teks += f"💸 <b>Total Uang Masuk:</b> <code>{format_rupiah(total_uang_msk)}</code>\n"
            ...
            if konteks_agregasi == "Tunggakan Terbanyak":
                ...
                top_debitors = sorted(pelanggan_stats.items(), key=lambda x: x[1]['tagihan'], reverse=True)
                for i, (nama, stats) in enumerate(top_debitors[:10], 1):
                    ringkasan_teks += f"{i}. <b>{nama}</b>: <code>{format_rupiah(stats['tagihan'])}</code>\n"
        elif konteks_agregasi == "Summary Uang Masuk" or konteks_agregasi == "Uang Masuk":
            ringkasan_teks += f"📥 <b>LAPORAN UANG MASUK (KAS)</b>\n"
            ringkasan_teks += f"💰 <b>Total Uang Masuk:</b> <code>{format_rupiah(total_uang_msk)}</code>\n"
            ringkasan_teks += f"📉 <b>Sisa Piutang:</b> <code>{format_rupiah(total_tagihan)}</code>\n"
            ...
            ringkasan_teks += f"💰 <b>Total Omzet (Kotor):</b> <code>{format_rupiah(total_omzet)}</code>\n"
            ringkasan_teks += f"💵 <b>Total Uang Masuk:</b> <code>{format_rupiah(total_uang_msk)}</code>\n"
            ...
            ringkasan_teks += f"⚠️ <b>Total Tagihan:</b> <code>{format_rupiah(total_tagihan)}</code>\n"
            ringkasan_teks += f"👥 <b>Jumlah Debitur:</b> {len(pelanggan_stats)}\n"
            ringkasan_teks += "━━━━━━━━━━━━━━━━━━━━━━\n"
            for nama, stats in pelanggan_stats.items():
                ringkasan_teks += f"👤 <b>{nama}</b>: <code>{format_rupiah(stats['tagihan'])}</code>\n"
...
def tangani_delete_data(chat_id, message_id_target):
    ...
    if not (f_nama or f_tanggal or f_barang or f_semua):
        safe_edit_message(ctx.bot, "❌ Gagal, mohon sebutkan kriteria spesifik (nama/tanggal/barang) atau perintah <b>'hapus semua data'</b>.", chat_id=chat_id, message_id=message_id_target, parse_mode="HTML")
        del ctx.user_sessions[chat_id]
        return
    ...
    safe_edit_message(ctx.bot, f"🗑️ Mencari data dengan parameter (Nama: {f_nama or '-'}, Tgl: {f_tanggal or '-'}, Brg: {f_barang or '-'}, Semua: {f_semua or '-'}) untuk dihapus...", chat_id=chat_id, message_id=message_id_target)
    ...
    try:
        ...
        for r in raw_data:
            ...
            row = [
                tgl_gs, nama_gs, barang_gs,
                str(r.get("jumlah_satuan", "")), str(r.get("harga", 0)), str(r.get("total", 0)),
                r.get("status", ""), r.get("metode_pembayaran", ""), str(r.get("tagihan", 0)), str(r.get("uang_masuk", 0))
            ]
            match_list.append({
                "row_index": r["id"],
                "nama": nama_gs,
    ...
    if len(match_list) == 0:
        safe_edit_message(ctx.bot, f"❌ Tidak ditemukan data penjualan yang sesuai dengan kriteria penghapusan tersebut.", chat_id=chat_id, message_id=message_id_target)
        del ctx.user_sessions[chat_id]
        ...
        if f_semua or len(match_list) > 10:
            ...
            for i, ml in enumerate(match_list, start=1):
                ...
                status_emoji = (
                    "✅" if "lunas" in str(status).lower() else
                    "⏳" if any(k in str(status).lower() for k in ["cicil", "dicicil", "dp"]) else
                    "🔴"
                )
                ...
                buttons.append(InlineKeyboardButton(f"🗑️ Hapus #{i}", callback_data=f"delres_{ml['row_index']}"))
                
            sess["hapus_data"] = match_list
            markup.add(*buttons)
            markup.row(InlineKeyboardButton("💣 Hapus Semua Terdeteksi", callback_data="btn_delete_bulk_trigger"))
            markup.add(InlineKeyboardButton("❌ Batalkan Semua", callback_data="btn_buang"))
            
            safe_edit_message(ctx.bot, teks_konflik, chat_id=chat_id, message_id=message_id_target, parse_mode="HTML", reply_markup=markup)
...
def siapkan_konfirmasi_delete(chat_id, message_id_target, info_row):
    ...
    safe_edit_message(ctx.bot, balasan, chat_id=chat_id, message_id=message_id_target, parse_mode="HTML", reply_markup=markup)
...
def siapkan_konfirmasi_delete_masal(chat_id, message_id_target, match_list):
    ...
    kriteria = []
    if ent.get("NAMA"): kriteria.append(f"Nama: {ent['NAMA']}")
    if ent.get("TANGGAL"): kriteria.append(f"Tgl: {ent['TANGGAL']}")
    if ent.get("BARANG"): kriteria.append(f"Brg: {ent['BARANG']}")
...
def siapkan_konfirmasi_update(chat_id, message_id_target, info_row):
    ...
    if ent_nlp.get("BARANG") and ent_nlp.get("BARANG").lower() != row[2].lower():
        ...
        if matches:
            ...
            ctx.bot.send_message(chat_id, f"💰 Produk diubah ke <b>{new_brg}</b>. Harga otomatis disesuaikan ke <code>{new_harga}</code>.", parse_mode="HTML")
        else:
            ctx.bot.send_message(chat_id, f"⚠️ Harga untuk <b>{new_brg}</b> tidak ditemukan di Master Data. Menggunakan harga lama.", parse_mode="HTML")
    ...
    safe_edit_message(ctx.bot, "🔄 Mempersiapkan Data...", chat_id=chat_id, message_id=message_id_target)
...
def tangani_catat_pelunasan(chat_id, message_id_target):
    ...
    if not f_nama:
        safe_edit_message(ctx.bot, 
            ...
            chat_id=chat_id, message_id=message_id_target, parse_mode="HTML"
        )
        del ctx.user_sessions[chat_id]
        return

    if not ctx.IS_DB_CONNECTED:
        safe_edit_message(ctx.bot, "❌ Mode Offline aktif.", chat_id=chat_id, message_id=message_id_target)
        del ctx.user_sessions[chat_id]
        return
    ...
    if not hutang_list:
        ...
        try:
            ...
            raw_data = db_client.get_transaksi_multi_filter(filters) if filters else db_client.get_semua_transaksi_db()
            ...
            for r in raw_data:
                ...
                logger.info(f"[DEBUG] Found candidate transaction: id={r['id']}, nama={nama_gs}, tagihan={tagihan_raw}")
    ...
    if len(hutang_list) == 1:
        ...
        safe_edit_message(ctx.bot, teks, chat_id=chat_id, message_id=message_id_target,
...
def _tampilkan_konfirmasi_pelunasan(chat_id, message_id_target, info_hutang, nominal_bayar):
    ...
    if nom_val > 0:
        ...
        status_ket   = "Lunas ✅" if tagihan_baru == 0 else f"Nyicil ⏳ (tagihan {format_rupiah(tagihan_baru)})"
    ...
    safe_edit_message(ctx.bot, balasan, chat_id=chat_id, message_id=message_id_target,
                           parse_mode="HTML", reply_markup=markup)
...
def tangani_revisi_manual(message):
    ...
    if chat_id not in ctx.user_sessions or ctx.user_sessions[chat_id].get("state") != "awaiting_edit_teks":
    ...
    if target_field == "NOMINAL_BAYAR" and sess.get("is_update_mode"):
        ...
        if "update_info" not in sess: sess["update_info"] = {}
    ...
    if target_field == "BARANG":
        ...
        if brg_ref:
            ...
            if matches:
                ...
                entitas["HARGA"] = format_rupiah(h_m["harga"])
                entitas["SATUAN"] = h_m["satuan"]
                sess["edit_notice"] = f"Harga otomatis berubah ke <code>{entitas['HARGA']}</code> ({h_m['satuan']})."
            else:
                sess["edit_notice"] = f"Satuan <b>{new_sat}</b> untuk <b>{brg_ref}</b> tidak ditemukan di Master Data."
...
def tangani_revisi_manual_multi(message):
    ...
    if target_field == "BARANG":
        ...
        if len(matches) == 1:
            ...
            entitas["SATUAN"] = h_m["satuan"]
            hitung_ulang_total_dinamis(entitas)
            sess["edit_notice"] = f"Item #{idx+1}: harga disesuaikan ke <code>{entitas['HARGA']}</code>."
        elif len(matches) > 1 and msg_lama_id:
            sess["multi_edit_index"] = idx
            tampilkan_pilihan_barang(chat_id, msg_lama_id, matches, callback_prefix=f"multi_pick_brg_{idx}")
            return
        else:
            sess["edit_notice"] = f"Item #{idx+1}: barang <b>{input_text}</b> tidak ditemukan di Master."
        ...
        if brg_ref:
            ...
            if matches:
                ...
                entitas["SATUAN"] = h_m["satuan"]
                hitung_ulang_total_dinamis(entitas)
                sess["edit_notice"] = f"Item #{idx+1}: harga berubah ke <code>{entitas['HARGA']}</code> ({h_m['satuan']})."
            else:
                sess["edit_notice"] = f"Item #{idx+1}: satuan <b>{input_text}</b> tidak ada untuk <b>{brg_ref}</b>."
...
def tangani_update_status(chat_id, message_id_target):
    ...
    if not (f_nama or f_tanggal or f_barang or f_semua):
        safe_edit_message(ctx.bot, "❌ Gagal, mohon sebutkan kriteria pesanan yang ingin diupdate (nama/tanggal/barang).", chat_id=chat_id, message_id=message_id_target)
        del ctx.user_sessions[chat_id]
        return
    ...
    safe_edit_message(ctx.bot, f"🔍 Mencari database untuk diupdate dengan parameter (Nama: {f_nama or '-'}, Tgl: {f_tanggal or '-'}, Brg: {f_barang or '-'}, Semua: {f_semua or '-'}) ...", chat_id=chat_id, message_id=message_id_target)
    ...
    try:
        ...
        for r in raw_data:
            ...
            if f_semua or (match_nama and match_barang):
                ...
                if match_unit:
                    ...
                    if entitas.get("STATUS") or entitas.get("METODE_PEMBAYARAN") or entitas.get("NOMINAL_BAYAR"):
        ...
        if date_exact and f_tanggal and len(match_list) == 0:
            raw_data = db_client.get_transaksi_multi_filter(filters_no_date) if filters_no_date else db_client.get_semua_transaksi_db()
            for r in raw_data:
                tgl_db = normalisasi_tanggal_gs(r.get("tanggal", ""))
                if not tgl_db or (f_tanggal != tgl_db):
                    continue
    ...
    if len(match_list) == 0:
        safe_edit_message(ctx.bot, f"❌ Tidak ditemukan pesanan yang sesuai kriteria tersebut untuk diubah.", chat_id=chat_id, message_id=message_id_target)
        del ctx.user_sessions[chat_id]
    elif len(match_list) == 1:
        # CEK: Jika ada status/nominal di entitas, lakukan AUTO-UPDATE tanpa tanya lagi
        if entitas.get("STATUS") or entitas.get("NOMINAL_BAYAR") or entitas.get("METODE_PEMBAYARAN"):
        ...
        for i, ml in enumerate(match_list[:10], start=1):
            ...
            buttons.append(InlineKeyboardButton(f"✏️ Ubah #{i}", callback_data=f"resolve_{ml['row_index']}"))
...
def lakukan_auto_update_data(chat_id, message_id_target, match_item, entitas):
    ...
    try:
        ...
        if new_nominal:
            ...
            proses_bayar_tambahan(ctx.db_transaksi, ctx.db_histori, row_idx, bayar_num, new_metode or old_metode)
        ...
        safe_edit_message(ctx.bot, msg_final, chat_id=chat_id, message_id=message_id_target, parse_mode="HTML")
        if chat_id in ctx.user_sessions: del ctx.user_sessions[chat_id]
        
    except Exception as e:
        safe_edit_message(ctx.bot, f"❌ Gagal auto-update: {e}", chat_id=chat_id, message_id=message_id_target)
...
def tangani_simpan_transaksi(chat_id, message_id_target):
    ...
    safe_debug_event({
        ...
        "data": {"chat_id": chat_id, "has_session": bool(sess), "has_entitas": bool((sess or {}).get("entitas")), "entitas": (sess or {}).get("entitas")},
    })
    if not sess or "entitas" not in sess:
        safe_edit_message(ctx.bot, "❌ Data tidak ditemukan. Silakan unggah foto kembali.", chat_id, message_id_target)
        return
    ...
    try:
        ...
        item_tagihan, item_uang_masuk, _ = hitung_sisa_tagihan(subtotal_item, nominal_bayar_item, status_pesanan)
        ...
        safe_debug_event({
            ...
            "data": {"chat_id": chat_id, "pesanan_id": pesanan_id, "data_pesanan": data_pesanan},
        })
        
        # 3. Simpan Pesanan Item
        b_id = None
        if ent.get("BARANG"):
            b_res = db_client.get_supabase().table("master_barang").select("id").ilike("nama_barang", f"%{ent['BARANG']}%").execute()
            if b_res.data:
...
def tangani_simpan_multi(chat_id, message_id_target):
    ...
    safe_debug_event({
        ...
        "data": {"chat_id": chat_id, "has_session": bool(sess), "multi_count": len((sess or {}).get("multi_results") or [])},
    })
    ...
    safe_edit_message(ctx.bot, f"⏳ Sedang menyimpan {len(results)} item ke Database Relasional...", chat_id, message_id_target)
    ...
    try:
        ...
        for res in results:
            ...
            nominal_bayar_item = int(ent.get("_CALC_UANG_MASUK", parse_rupiah(ent.get("NOMINAL_BAYAR") or 0)) or 0)
            item_tagihan = int(ent.get("_CALC_TAGIHAN", max(0, subtotal_item - nominal_bayar_item)) or 0)
        ...
        safe_debug_event({
            ...
            "data": {"chat_id": chat_id, "tanggal": tanggal_pesanan, "count": len(flat_inserted), "flat_inserted": flat_inserted},
        })
        ...
        safe_edit_message(ctx.bot, f"❌ Gagal menyimpan batch ke skema baru: {e}", chat_id, message_id_target)
