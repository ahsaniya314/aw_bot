...
def tangani_tambah_barang_chat(chat_id, message_id_target):
    ...
    if nama and str(nama).strip().lower() in {
        "permen",
        "serbuk",
        "roti",
        "pia",
        ...
        mapping = [
            ...
            (
                ["bemmbeng", "bembeng", "bengbeng", "beng-beng", "beng beng", "beem-beeng"],
                "Bembeng",
            ),
    ...
    if nama and harga_raw and satuan_raw:
        try:
            ...
            exists = any(
                b["nama"].lower() == nama.lower() and b["satuan"].lower() == satuan_raw.lower()
                for b in semua
            )
            if exists:
                safe_edit_message(
                    bot,
                    f"⚠️ Barang <b>{nama}</b> dengan satuan <b>{satuan_raw}</b> sudah ada. Gunakan menu 'Set Harga' untuk mengubahnya.",
                    chat_id,
                    message_id_target,
                    parse_mode="HTML",
                )
            ...
            if "23505" in err_str or "duplicate key" in err_str.lower():
                ...
                safe_edit_message(
                    bot,
                    f"❌ <b>Gagal menyimpan barang</b>\n\nDetail error: <code>{err_str}</code>",
                    chat_id,
                    message_id_target,
                    parse_mode="HTML",
                )
    ...
    if not nama:
        ...
        safe_edit_message(
            bot,
            "➕ <b>Barang apa yang ingin Anda tambahkan?</b>\nContoh: <code>Permen Coklat</code>",
            chat_id,
            message_id_target,
            parse_mode="HTML",
            reply_markup=markup,
    ...
    msg = safe_edit_message(
        bot,
        f"💰 Berapa harga default untuk <b>{nama}</b>?\n(Ketik angka saja, mis: <code>15000</code>)",
        chat_id,
        message_id_target,
        parse_mode="HTML",
        reply_markup=markup,
...
def _mb_terima_nama(message):
    ...
    if chat_id not in user_sessions or user_sessions[chat_id].get("state") == "cancelled":
        return
...
def _mb_terima_harga(message):
    ...
    user_sessions = ctx.user_sessions
    chat_id = message.chat.id
    if chat_id not in user_sessions or "mb_nama_baru" not in user_sessions[chat_id]:
        return
    if user_sessions[chat_id].get("state") == "cancelled":
        return
    if message.text.startswith("/"):
        return
    ...
    btn_list = [
        InlineKeyboardButton(unit.capitalize(), callback_data=f"mb_unit_{unit}")
        for unit in satuan_list_sorted
    ]
...
def _mb_terima_satuan(message, chat_id=None, message_id_target=None):
    ...
    try:
        ...
        pass
    if not message_id_target:
        message_id_target = user_sessions[chat_id].get("mb_msg_id")
    satuan = ""
    if message:
        if message.text.startswith("/"):
            return
    ...
    try:
        semua = get_all_barang(ctx.db_barang)
        exists = any(
            b["nama"].lower() == nama.lower() and b["satuan"].lower() == satuan.lower()
            for b in semua
        )
        if exists:
            safe_edit_message(
                bot,
                f"⚠️ Barang <b>{nama}</b> dengan satuan <b>{satuan}</b> sudah ada. Gunakan menu 'Set Harga' untuk mengubahnya.",
                chat_id,
                message_id_target,
                parse_mode="HTML",
            )
        ...
        if "23505" in err_str or "duplicate key" in err_str.lower():
            safe_edit_message(
                bot,
                f"⚠️ <b>GAGAL MENYIMPAN BARANG</b>\n\nNama barang <b>{nama}</b> sudah ada di database.\n\nSilakan ikuti petunjuk fix database atau tambahkan pembeda di nama produk.",
                chat_id,
                message_id_target,
                parse_mode="HTML",
            )
...
def tangani_cek_harga_chat(chat_id, message_id_target):
    ...
    if not nama_cari:
        ...
        safe_edit_message(
            bot, teks, chat_id, message_id_target, parse_mode="HTML", reply_markup=markup
        )
    else:
        hasil_list = cari_harga_default(
            ctx.db_barang, nama_cari, satuan_cari=satuan_cari, semua_barang=semua
        )
        if hasil_list:
            if len(hasil_list) == 1:
                hasil = hasil_list[0]
                teks = (
                    f"🔍 <b>Informasi Harga Barang</b>\n━━━━━━━━━━━━━━━━━━━━━━\n"
            ...
            safe_edit_message(
                bot,
                f"❌ Barang <b>{nama_cari}</b> tidak ditemukan di Master Barang.",
                chat_id,
                message_id_target,
                parse_mode="HTML",
            )
...
def tangani_set_harga_chat(chat_id, message_id_target):
    ...
    if chat_id not in user_sessions or "entitas" not in user_sessions[chat_id]:
        safe_edit_message(
            bot,
            "❌ Sesi kedaluwarsa. Silakan unggah foto barang kembali.",
            chat_id,
            message_id_target,
        )
        return
        ...
        for b in semua:
            markup.add(
                InlineKeyboardButton(
                    f"✏️ {b['nama']} ({format_rupiah(b['harga'])})",
                    callback_data=f"mb_edit_{b['row_idx']}",
                )
            )
        markup.add(InlineKeyboardButton("❌ Batal", callback_data="mb_batal"))
        safe_edit_message(
            bot,
            "Pilih barang yang ingin diatur harganya:",
            chat_id,
            message_id_target,
            reply_markup=markup,
        )
    ...
    if not harga_raw:
        ...
        if not hasil_list:
            safe_edit_message(
                bot,
                f"❌ Barang <b>{nama}</b> tidak ditemukan.",
                chat_id,
                message_id_target,
                parse_mode="HTML",
            )
        ...
        if len(hasil_list) > 1:
            ...
            row_idx = next(
                (
                    b["row_idx"]
                    for b in semua
                    if b["nama"] == h["nama"] and b["satuan"] == h["satuan"]
                ),
                None,
            )
            ...
            user_sessions[chat_id][
                "mb_msg_id"
            ] = message_id_target  # Setup msg_id untuk callback nanti
    ...
    if len(hasil_list) == 1:
        ...
        semua = get_all_barang(ctx.db_barang)
        row_idx = next(
            (b["row_idx"] for b in semua if b["nama"] == h["nama"] and b["satuan"] == h["satuan"]),
            None,
        )
        user_sessions[chat_id]["mb_edit_row"] = row_idx
        markup = InlineKeyboardMarkup()
        ...
        for i, m in enumerate(hasil_list):
            markup.add(
                InlineKeyboardButton(
                    f"✏️ Update {m['nama']} ({m['satuan']})", callback_data=f"mb_apply_price_{i}"
                )
            )
    ...
    if not nama:
        ...
        for b in semua:
            markup.add(
                InlineKeyboardButton(
                    f"🗑️ {b['nama']} ({format_rupiah(b['harga'])})",
                    callback_data=f"mb_hapus_prep_{b['row_idx']}",
                )
            )
        markup.add(InlineKeyboardButton("❌ Batal", callback_data="mb_batal"))
        safe_edit_message(
            bot, "Pilih barang yang ingin dihapus:", chat_id, message_id_target, reply_markup=markup
        )
        return

    semua = get_cached_barang() if ctx.IS_DB_CONNECTED else []
    hasil_list = cari_harga_default(
        ctx.db_barang, nama, satuan_cari=satuan_cari, semua_barang=semua
    )
    if not hasil_list:
        safe_edit_message(
            bot,
            f"❌ Barang <b>{nama}</b> tidak ditemukan di Master Data.",
            chat_id,
            message_id_target,
            parse_mode="HTML",
        )
...
def _mb_terima_nama_edit_single(message):
    ...
    try:
        ...
        pass
    if chat_id not in user_sessions or "mb_edit_row" not in user_sessions[chat_id]:
        return
    ...
    try:
        ...
        if target:
            ...
            bot.send_message(
                ...
                f"✨ <b>Satuan Baru:</b> <b>{unit_baru}</b>",
                parse_mode="HTML",
            )
            try:
                bot.delete_message(chat_id, msg_id)
...
def _mb_terima_harga_edit_wizard(message):
    ...
    try:
        ...
        markup = InlineKeyboardMarkup(row_width=3)
        btn_list_wizard = [
            InlineKeyboardButton(unit.capitalize(), callback_data=f"mb_wizard_unit_{unit}")
            for unit in satuan_list_wizard
        ]
