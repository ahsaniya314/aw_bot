...
def split_multi_entries(text):
    ...
    text_norm = re.sub(
        r"\s+dan\s+(?=hari ini|kemarin|besok|tgl|tanggal|\d{1,2}/\d{1,2})",
        " | ",
        text,
        flags=re.IGNORECASE,
    )
    ...
    text_norm = re.sub(
        rf"({unit_words})(?={action_words}\b)", r"\1 | ", text_norm, flags=re.IGNORECASE
    )
    text_norm = re.sub(
        rf"({unit_words})\s*(?={action_words}\b)", r"\1 | ", text_norm, flags=re.IGNORECASE
    )
...
def _apply_multi_overrides(results):
    ...
    for group in groups:
        ...
        items = [
            r
            for r in group
            if r.get("entitas", {}).get("BARANG") and r.get("entitas", {}).get("JUMLAH")
        ]
        if not items:
            continue
        ...
        if not global_status and len(items) > 1:
            ...
            if last_item:
                ...
                if st_last and all_no_status:
                    ...
                    m_last = last_item["entitas"].get(
                        "METODE_PEMBAYARAN"
                    ) or _detect_method_from_text(last_item.get("original_text") or "")
                    if m_last:
            ...
            for it in items:
                ...
                if global_status:
                    ...
                    if global_status == "Dicicil" and global_frac:
                        ...
                        e_it["NOMINAL_BAYAR"] = (
                            "SETENGAH" if float(global_frac) == 0.5 else e_it.get("NOMINAL_BAYAR")
                        )
                ...
                if _match_item_in_modifier(it["entitas"], override_text):
                    if status_override:
                        ...
                        if status_override == "Dicicil" and frac_override:
                            ...
                            it["entitas"]["NOMINAL_BAYAR"] = (
                                "SETENGAH"
                                if float(frac_override) == 0.5
                                else it["entitas"].get("NOMINAL_BAYAR")
                            )
                    if metode_override:
...
def _detect_item_in_text(segment, daftar_barang=None):
    ...
    match_js = re.search(
        r"(\d+)\s*(dus|pcs|toples|pack|bks|buah|botol|karton|kg|bungkus|renceng|bal|box|lusin|roll|sachet|pouch|butir|kantong|koli|lembar|meter|ikat|can|kaleng|gelas|cup|tablet|kapsul)\b",
        seg_lower,
    )
    if match_js:
...
def parse_multi_item_with_status(teks_user, daftar_barang=None):
    ...
    nama_match = re.search(
        r"(?:pak|bu|mas|mbak|kak|ibu|bapak)\s+([a-z]+(?:\s+[a-z]+)?)", order_section
    )
    if nama_match:
    ...
    return {"nama": nama_pelanggan, "tanggal": tanggal, "items": order_items}
...
def proses_nlp(
    teks_user, db_metode=None, daftar_barang=None, mapping_metode=None, already_normalized=False
):
    """
    ...
    if multi_parsed and multi_parsed.get("items"):
        ...
        for item in multi_parsed["items"]:
            ...
            intent = (
                "Catat_Penjualan_Lunas" if entitas["STATUS"] == "Lunas" else "Catat_Penjualan_Cicil"
            )

            results.append({"intent": intent, "entitas": entitas, "original_text": teks_user})
    ...
    for entry in entries:
        teks_koreksi = (
            entry if already_normalized else koreksi_teks(entry, daftar_barang=daftar_barang)
        )
        hasil = ekstrak_entitas(
            teks_koreksi,
            teks_asli=entry,
            db_metode=db_metode,
        ...
        if len(entries) > 1:
            ...
            if (
                ...
                and not entitas.get("BARANG")
                and not entitas.get("STATUS")
                and not entitas.get("METODE_PEMBAYARAN")
                and not entitas.get("SEMUA")
            ):
                continue

        results.append({"intent": intent, "entitas": entitas, "original_text": entry})
    ...
    if not results and teks_user.strip():
        teks_koreksi = (
            ...
            else koreksi_teks(teks_user, daftar_barang=daftar_barang)
        )
        hasil = ekstrak_entitas(
            teks_koreksi, teks_asli=teks_user, db_metode=db_metode, daftar_barang=daftar_barang
        )
