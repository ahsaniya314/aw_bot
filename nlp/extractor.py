...
_MASTER_BARANG_UNITS = {
    "karton", "toples", "pouch", "pack", "bungkus", "keranjang",
    "pcs", "dus", "pak", "box", "buah", "botol", "kg", "lusin", "sachet", "kantong"
}
_TRANSAKSI_HINT_WORDS = {
    "ambil", "pesan", "beli", "order", "orderan", "catat",
    "pak", "bu", "mas", "mbak", "kak", "a.n", "atas nama",
    "lunas", "hutang", "utang", "dicicil", "cicil", "dp", "uang muka",
    "bayar", "tunai", "transfer", "qris"
}
...
def _detect_short_set_harga(teks_lower, daftar_barang):
    if not teks_lower:
        return None
    if any(w in teks_lower for w in ["tambah", "tambahkan", "tamhbahkan", "input", "bikin", "baru", "add"]):
        return None
    ...
    unit_match = None
    m_unit = re.search(r"\b(" + "|".join(sorted(_MASTER_BARANG_UNITS, key=len, reverse=True)) + r")\b", teks_lower)
    if m_unit:
    ...
    harga_candidates = []
    for m in re.finditer(r"(rp\.?\s*)?(\d[\d\.,]*)\s*(juta|jt|jutaan|ribu|rb|ribuan|k)?\b", teks_lower):
        ...
        is_price_like = has_rp or bool(suffix) or (digits_only and len(digits_only) >= 4) or ("=" in teks_lower) or ("harga" in teks_lower)
        if is_price_like:
    ...
    if daftar_barang:
        exists_in_master = any(str(x).strip().lower() == str(barang_match).strip().lower() for x in daftar_barang)
...
def ekstrak_entitas(teks_koreksi, teks_asli="", db_metode=None, daftar_barang=None, mapping_metode=None):
    ...
    bulan_dict = {
        ...
        "agustus": "08", "agu": "08", "agus": "08", "september": "09", "sep": "09",
        "oktober": "10", "okt": "10",
        "november": "11", "nov": "11", "nop": "11", "nopember": "11",
        "desember": "12", "des": "12"
    }
    slang_map = {
        r"\bdel\b": "hapus", r"\bilangin\b": "hapus", r"\bbuang\b": "hapus", r"\bbatalin\b": "hapus", r"\bapush\b": "hapus",
        r"\bkmn\b": "kemarin", r"\bkmarin\b": "kemarin", r"\bkmrin\b": "kemarin", r"\bkmaren\b": "kemarin",
        r"\bhri\b": "hari", r"\bhri ini\b": "hari ini", r"\bhari inj\b": "hari ini", r"\bh4ri\b": "hari", r"\bhr ini\b": "hari ini", r"\bhr ni\b": "hari ini",
        r"\bbsk\b": "besok", r"\bbsok\b": "besok",
        r"\bpk\b": "pak", r"\bbpk\b": "pak", r"\bp4k\b": "pak",
        r"\bdy\b": "dia", r"\bdya\b": "dia",
        r"\budh\b": "sudah", r"\bsdh\b": "sudah", r"\budah\b": "sudah",
        r"\bbyr\b": "bayar", r"\bbyar\b": "bayar", r"\bbyarnya\b": "bayar", r"\bnitip\b": "bayar",
        r"\bstngh\b": "setengah", r"\bstngah\b": "setengah", r"\bseparo\b": "setengah",
        r"\bpke\b": "pakai", r"\bpkai\b": "pakai",
        r"\btrnsfer\b": "transfer", r"\btrnsfr\b": "transfer", r"\btf\b": "transfer", r"\btranfer\b": "transfer", r"\btrf\b": "transfer",
        r"\bbln\b": "bulan", r"\bthn\b": "tahun",
        r"\btgl\b": "tanggal",
        r"\btmpilin\b": "tampilkan", r"\btampilin\b": "tampilkan", r"\btampilan\b": "tampilkan", r"\btampil\b": "tampilkan", r"\btmmpilin\b": "tampilkan", r"\bliat\b": "tampilkan", r"\bliatin\b": "tampilkan", r"\brincian\b": "tampilkan",
        r"\bksih tau\b": "tampilkan", r"\bkasih tau\b": "tampilkan", r"\bksih\b": "kasih",
        r"\bpesenan\b": "pesanan", r"\bpesnan\b": "pesanan", r"\bpesanannya\b": "pesanan", r"\bpsnn\b": "pesanan", r"\bpsnan\b": "pesanan", r"\bpesenane\b": "pesanan",
        r"\blunasin\b": "lunasi", r"\blunaskan\b": "lunasi", r"\bngelunasin\b": "lunasi", r"\blns\b": "lunas", r"\blnas\b": "lunas", r"\bnutup\b": "lunasi", r"\bkurangin tagihane\b": "lunasi",
        r"\byg\b": "yang", r"\bdgn\b": "dengan", r"\bsm\b": "sama",
        r"\bgk\b": "tidak", r"\bgak\b": "tidak", r"\bga\b": "tidak",
        r"\brb\b": "ribu", r"\bjt\b": "juta",
        r"\bbrg\b": "barang", r"\bprng\b": "barang", r"\bparang\b": "barang", r"\bitm\b": "item", r"\bprod\b": "produk",
        r"\bspa\b": "siapa", r"\bsapa\b": "siapa", r"\bsiapa aja\b": "siapa", r"\bsiapa saja\b": "siapa",
        r"\bbrp\b": "berapa", r"\bjml\b": "jumlah", r"\btotalan\b": "total", r"\bttal\b": "total",
        r"\bmasih nyicil\b": "nyicil", r"\bblom\b": "belum", r"\bblm\b": "belum",
        # --- Tambahan slang baru ---
        r"\bdicil\b": "dicicil", r"\bdicill\b": "dicicil", r"\bdicl\b": "dicicil",
        r"\bkasbon\b": "hutang dulu", r"\bkasbon2\b": "hutang dulu",
        r"\bbon dulu\b": "hutang dulu",
        r"\butang\b": "hutang", r"\butang dulu\b": "hutang dulu", r"\bhutan\b": "hutang", r"\bngutang\b": "hutang",
        ...
        r"\bctn\b": "karton", r"\betn\b": "karton", r"\bcrn\b": "karton", r"\bkrtn\b": "karton",
        r"\bkartun\b": "karton", r"\bkraton\b": "karton", r"\bkaton\b": "karton", r"\bkatron\b": "karton",
        r"\bkrton\b": "karton", r"\bkerton\b": "karton", r"\bkartom\b": "karton",
        r"\bbks\b": "bungkus",
        r"\btunai\b": "tunai", r"\bcash\b": "tunai", r"\bkontan\b": "tunai", r"\bcsh\b": "tunai",
        r"\b(?:di)?bayar langsung\b": "tunai", r"\blangsung\s+(?:di)?bayar\b": "tunai",
        r"\bganti harga\b": "update harga", r"\bubah harga\b": "update harga", r"\bedti\b": "edit", r"\bupdet\b": "update", r"\bupdt\b": "update", r"\bralat\b": "update",
        r"\btmbhin\b": "tambah", r"\bmasukin\b": "tambah", r"\bbikin\b": "tambah", r"\btmb\b": "tambah", r"\badd\b": "tambah",
        r"\btamhbahkan\b": "tambah", r"\btamhahkan\b": "tambah", r"\btambahin\b": "tambah",
        r"\bhr ini\b": "hari ini", r"\bhr ni\b": "hari ini",
        r"\blaproan\b": "laporan", r"\blapuran\b": "laporan", r"\blaporan\b": "laporan",
        r"\bmnggu ini\b": "minggu ini", r"\bplg gede\b": "paling banyak",
    }
    ...
    try:
        ...
        _group_to_replacement = {
            "aksi_tambah": "tambah", "aksi_tampilkan": "tampilkan",
            "aksi_hapus": "hapus", "aksi_edit": "edit", "aksi_cek": "cek",
            "status_lunas": "lunas", "status_belum_lunas": "belum lunas",
            "metode_cash": "tunai", "metode_transfer": "transfer",
        }
    ...
    teks_prenorm = teks_prenorm.replace("5o", "50").replace("4gus", "agus").replace("0rder", "order")\
                               .replace("1lo", "110").replace("3o", "30").replace("10o", "100")\
                               .replace("t0t0k", "totok")
    ...
    teks_asli_lower = (teks_asli or teks_koreksi).lower()  # Untuk deteksi JUMLAH/HARGA (angka dari teks asli)
    ...
    if dataset_hint:
        ...
        elif _mapped in ("Read_Analitik_Penjualan", "Read_Analitik_Hutang", "Read_Transaksi_Spesifik"):
            entitas["AKSI"] = "Read Data"
        elif _mapped == "CRUD_Barang":
            if "tambah" in intent: entitas["AKSI"] = "Tambah Barang"
            elif "edit" in intent: entitas["AKSI"] = "Set Harga Barang"
            elif "hapus" in intent: entitas["AKSI"] = "Hapus Barang"
            else: entitas["AKSI"] = "Cek Harga Barang"
        elif _mapped == "Update_Delete_Transaksi":
            if "hapus" in intent: entitas["AKSI"] = "Delete Data"
            else: entitas["AKSI"] = "Update Status"
        elif _mapped == "Chit_Chat":
            entitas["AKSI"] = "Chit Chat"
        # Also handle legacy intent name format (backward compat)
        elif "Catat_Penjualan" in intent: entitas["AKSI"] = "Tambah Penjualan"
        elif "Pelunasan" in intent: entitas["AKSI"] = "Catat Pelunasan"
        elif "Read_Transaksi_Analitik" in intent: entitas["AKSI"] = "Read Data"
        elif "Read_Transaksi_Spesifik" in intent: entitas["AKSI"] = "Read Data"
        elif "Create_Barang" in intent: entitas["AKSI"] = "Tambah Barang"
        elif "Update_Barang" in intent: entitas["AKSI"] = "Set Harga Barang"
        elif "Delete_Barang" in intent: entitas["AKSI"] = "Hapus Barang"
        
        # Ekstrak entitas dari hint jika ada
        hint_entities = dataset_hint.get("entities", {})
        if hint_entities.get("nama_pelanggan"): entitas["NAMA"] = hint_entities["nama_pelanggan"].title()
        elif hint_entities.get("pelanggan"): entitas["NAMA"] = hint_entities["pelanggan"].title()
    ...
    if short_set and (not entitas.get("AKSI") or entitas.get("AKSI") == "Tambah Penjualan") and not entitas.get("NAMA"):
        entitas["AKSI"] = "Set Harga Barang" if short_set.get("exists_in_master") else "Tambah Barang"
    ...
    match_hari_lagi = re.search(r"(\d+)(?:\s*-\s*(\d+))?\s+(hari|minggu)\s+(yang\s+)?(besok|lagi|kedepan|ke depan)", teks_lower)
    match_hari_lalu = re.search(r"(\d+)(?:\s*-\s*(\d+))?\s+(hari|minggu)\s+(yang\s+)?(kemarin|lalu)", teks_lower)
    
    # 2. Deteksi Nama Hari relatif
    match_nama_hari = re.search(r"(kemarin|besok|lalu|depan)\s+(?:hari\s+)?(senin|selasa|rabu|kamis|jumat|sabtu|minggu)|(?:hari\s+)?(senin|selasa|rabu|kamis|jumat|sabtu|minggu)\s+(kemarin|lalu|depan|besok)", teks_lower)
    ...
    if "kemarin" in teks_lower or "semalam" in teks_lower:
        ...
        delta_val = int(match_hari_lalu.group(2) if match_hari_lalu.group(2) else match_hari_lalu.group(1))
        unit = match_hari_lalu.group(3)
        if "minggu" in unit: delta_val *= 7
        ...
        delta_val = int(match_hari_lagi.group(2) if match_hari_lagi.group(2) else match_hari_lagi.group(1))
        ...
        match_bulan_teks = re.search(rf"(\d{{1,2}})\s+({bulan_pattern})(?:\s+(\d{{2,4}}))?", teks_lower)
        ...
        if match_bulan_teks:
            ...
            d, m, y = int(match_tgl_full.group(1)), int(match_tgl_full.group(2)), int(match_tgl_full.group(3))
            if y < 100: y += 2000
            entitas["TANGGAL"] = f"{d:02d}-{m:02d}-{y}"
        elif match_tgl_rapat:
            d, m, y = int(match_tgl_rapat.group(1)), int(match_tgl_rapat.group(2)), int(match_tgl_rapat.group(3))
            entitas["TANGGAL"] = f"{d:02d}-{m:02d}-{y}"
        elif match_tgl_saja:
    ...
    if any(k in teks_lower for k in ["dashboard", "laporan harian", "rekap harian", "ringkasan harian"]):
        entitas["KONTEKS_AGREGASI"] = "Dashboard Harian"
        entitas["AKSI"] = "Read Data"
        entitas["SEMUA"] = True
        if not entitas["TANGGAL"]: entitas["TANGGAL"] = hari_ini.strftime("%d-%m-%Y")

    elif any(k in teks_lower for k in ["terbanyak", "paling banyak", "plg gede", "terbangak", "paking banyak"]):
    ...
    elif any(k in teks_lower for k in ["tagihan", "piutang", "tunggakan", "belum bayar", "sisa bayar", "kekurangan", "sisa tagihan", "total hutang", "berapa utang", "berapa tagihan"]):
    ...
    elif "semua" in teks_lower and any(k in teks_lower for k in ["transaksi", "penjualan", "pesanan", "order", "data"]):
        entitas["SEMUA"] = True
        entitas["AKSI"] = "Read Data"
    ...
    if any(k in teks_lower for k in ["set harga", "ubah harga", "ganti harga", "edit harga", "naikin jd"]):
        entitas["AKSI"] = "Set Harga Barang"
    elif any(k in teks_lower for k in ["tambah barang", "barang baru", "tambah parang", "input barang", "input produk", "tambah produk", "tambah item", "add", "bikin master"]):
        entitas["AKSI"] = "Tambah Barang"
    elif any(k in teks_lower for k in ["hapus barang", "hapus data barang", "hapus produk", "buang barang", "delete barang", "hapus item", "delete item"]):
    ...
    elif any(k in teks_lower for k in ["tampilkan", "lihat", "baca", "cari", "cek", "rekap", "riwayat", "laporan", "tampilan", "tampil", "info", "kasih tau", "siapa", "sapa", "berapa"]):
        entitas["AKSI"] = "Read Data"
    ...
    if not entitas["AKSI"]:
        if any(k in teks_lower for k in ["tambah", "input", "catat", "order", "orderan", "ordran", "beli", "pembelian", "ambil", "pesan", "pesanan"]):
            entitas["AKSI"] = "Tambah Penjualan"
        else:
    ...
    # B. Cari NAMA Pintar
    ignore_for_name = {
        "siapa", "apa", "mana", "kapan", "berapa", "bagaimana", "kenapa", "saja", "aja",
        "ambil", "order", "pesan", "beli", "tambah", "masukkan", "catat", "baru", "bikin", "mesen", "pesen",
        "hapus", "delete", "buang", "remove", "menghapus", "batalkan", "cancel",
        "update", "edit", "ubah", "revisi", "ganti", "perbaharui", "perbarui", "mengatur", "setel", "setting", "pengaturan", "set", "atur",
        "bayar", "bayarnya", "dibayar", "dibayarkan", "membayar",
        "lunasi", "melunasi", "dilunasi", "lunasin", "lunasinnya", "lunas",
        "cicil", "dicicil", "nyicil", "nyicilnya", "mencicil", "mencicilnya", "cicilan",
        "dp", "ngutang", "hutang", "utang", "kasbon", "kredit", "hutan", "tagihan", "tagihannya",
        "belum", "sudah", "telah", "lunas", "lunasin", "sudah bayar",
        "tampilkan", "berikan", "lihat", "baca", "cari", "cek", "riwayat", "laporan", "rekap", "tampilan", "tampil",
        "data", "transaksi", "semua", "kasih", "info",
        "pesanan", "orderan", "nota", "bon", "struk",
        "pembayaran", "pelunasan", "cicilan", "piutang", "tunggakan", "tunggakannya",
        "sisa", "sisanya", "kekurangan", "kekurangannya",
        "uang", "muka", "separuh", "setengah", "setengahnya",
        "permen", "coklat", "millo", "cokelat", "prmen", "lolipop", "lalipop", "loli",
        "roti", "pia", "potong", "roda", "bulus", "bakpia", "pita", "brownis", "bronis", "brownies",
        "makanan", "serbuk", "mknan", "srbk", "meses", "meises", "mses", "mesis",
        "dus", "pcs", "kg", "pack", "bks", "buah", "botol", "karton", "bungkus", "toples", "renceng",
        "harga", "harganya", "rp", "rupiah", "total", "jadi", "jumlah", "satuan",
        "sebanyak", "sejumlah", "senilai", "sebesar", "sejumlah",
        "atas", "nama", "an",
        "hari", "ini", "besok", "kemarin", "lusa", "minggu", "bulan", "tahun",
        "tanggal", "tgl", "pagi", "siang", "sore", "malam", "sekarang", "kemudian", "nanti",
        "yang", "di", "ke", "dari", "buat", "untuk", "sama", "dengan", "dan", "atau",
        "tolong", "mohon", "coba", "dong", "ya", "lah", "pun", "naha",
        "secara", "langsung", "segera", "sebagian", "sepenuhnya", "seluruhnya",
        "otomatis", "manual", "via", "pakai", "pake",
        "ada", "tidak", "gak", "enggak", "nya", "punya", "kalo", "kalau", "bisa", "mau",
        "juga", "lagi", "saja", "aja", "sih", "kok", "deh", "kan", "pas",
        "tunai", "transfer", "cash", "qris", "bca", "bri", "mandiri", "gopay", "dana", "ovo", "shopeepay",
        "dia", "sedang", "diambil", "dipesan", "pre",
        "penjualan", "pembelian", "pengiriman", "catatan", "daftar", "list", "record",
        "toko", "barang", "produk", "item", "stok", "stock", "dashboard", "laporan", "ringkasan", "summary", "rekap",
        "paling", "banyak", "terbanyak", "teratas", "tertinggi", "pembeli", "pelanggan", "masih",
        "masuk", "pemasukan", "keluar", "omzet", "terbangak", "paking", "berpa", "brapaan", "siapa", "sapa",
        "laproan", "lapuran", "penjualan",
        "inj", "inih", "ini", "itu", "ituu", "tadi", "tadih", "pesan", "psn"
    }

    entitas["NAMA"] = None
    
    # 1. Deteksi Nama dengan Prefix (Prioritas Tinggi)
    match_prefix = re.search(r"(pak|bu|mas|mbak|kak|atas nama|a\.n\.?)\s+([a-z]+)(?:\s+([a-z]+))?", teks_lower)
    if match_prefix:
    ...
    if not entitas["NAMA"] and entitas["AKSI"] not in ["Cek Harga Barang"]:
        ...
        is_global_report = any(k in teks_lower for k in ["dashboard", "rekap harian", "ringkasan harian", "pembeli terbanyak", "tunggakan terbanyak", "siapa yang", "total", "berapa", "berpa", "semua data", "data penjualan", "semua penjualan"])
        ...
        if not is_global_report:
            ...
            for w in words:
                ...
                if (w not in ignore_for_name and 
                    not w.isdigit() and 
                    len(w) >= 3 and 
                    w not in bulan_dict and 
                    not re.search(r'\d+[kjt]', w) and
                    not any(w.lower() == k.lower() for k in DAFTAR_KATA_KUNCI)):
                    
                    nama_kandidat.append(w)
                    if len(nama_kandidat) >= 2: break
            
            if nama_kandidat:
                # Pastikan nama tidak mengandung kata status atau kuantitas yang mungkin lolos
                forbidden_name_words = [
                    "lunas", "cicil", "hutang", "tunai", "transfer", "nyicil", "mencicil",
                    "juta", "jtan", "jt", "ribu", "rb", "rban", "ratus", "puluh",
                    "dus", "pcs", "toples", "pack", "bks", "buah", "botol", "karton", "ctn", "etn", "crn", "krtn", "kg",
                    "bal", "butir", "kantong", "lusin", "koli", "roll", "meter", "lembar", "box", "bungkus",
                    "ambil", "beli", "order", "pesan", "catat", "bayar", "nominal", "total", "satuan",
                    "hari", "ini", "besok", "kemarin", "lusa", "tanggal", "tgl"
                ]
    ...
    if kandidat:
        ...
        if chosen_val:
            if (not entitas["BARANG"]) or (
                entitas["BARANG"] and _is_generic_barang(entitas["BARANG"]) and not _is_generic_barang(chosen_val)
            ):
                entitas["BARANG"] = chosen_val
                barang_posisi = teks_lower.find(chosen_kw)
    ...
    if match_satuan:
        ...
        if match_jml_prefix:
            ...
            if not entitas["JUMLAH"]:
                ...
                teks_jml = teks_qty.replace("5o", "50").replace("1lo", "110").replace("3o", "30").replace("10o", "100")
                ...
                for num in angka_murni:
                    ...
                    if pre_text and pre_text[-1] in ["harga", "rp", "senilai", "sebesar", "nominal", "per", "satuan"]:
                        continue
                    entitas["JUMLAH"] = num
                    break
    ...
    match_harga = re.search(r"(?:harga|satuan|per)\b.*?\b(?:rp\s*)?([\d\.,]+(?:\s*(?:k|jt|juta|rb|ribu|jutaan))?)\b", teks_asli_lower)
    if match_harga: 
        val = parse_price_shorthand(match_harga.group(1))
        if val: entitas["HARGA"] = format_rupiah(val)
    
    match_total = re.search(r"(?:total|jadi|bayar)\b.*?\b(?:rp\s*)?([\d\.,]+(?:\s*(?:k|jt|juta|rb|ribu|jutaan))?)\b", teks_asli_lower)
    if match_total: 
        val = parse_price_shorthand(match_total.group(1))
        if val: entitas["TOTAL"] = format_rupiah(val)

    # F. NOMINAL BAYAR (Extract first to help determine status)
    # Mendukung variasi 'cicil', 'mencicil', 'nyicil', 'dp', 'bayar', dll
    match_nom = re.search(r"(?:bayar|dp|cicil|dicil|mencicil|nyicil|nominal|sejumlah|sebesar|nominalnya|bayarnya|pembayaran)\b.*?\b(?:rp\s*)?([\d\.,]+(?:\s*(?:k|jt|juta|rb|ribu|jutaan))?)\b", teks_asli_lower)
    if match_nom:
        val = parse_price_shorthand(match_nom.group(1))
        if val: entitas["NOMINAL_BAYAR"] = format_rupiah(val)
    ...
    if any(k in teks_lower for k in ["cicil", "dicil", "nyicil", "dp", "uang muka", "separuh", "bayar dulu", "bayar tagihan", "bayar hutang", "pelunasan"]):
        entitas["STATUS"] = "Dicicil"
        match_nom = re.search(r"(?:bayar|dp|cicil|dicil|nominal|sejumlah|sebesar|pelunasan)\b.*?\b(?:rp\s*)?([\d\.,]+(?:\s*(?:k|jt|juta|rb|ribu|jutaan))?)\b", teks_asli_lower)
        if match_nom:
            val = parse_price_shorthand(match_nom.group(1))
            if val:
    ...
    elif any(k in teks_lower for k in ["hutang", "utang", "kasbon", "kredit", "bon dulu", "nanti bayar", "belum bayar", "belum lunas", "ngutang", "tunggakan", "piutang"]):
        entitas["STATUS"] = "Hutang"
    elif any(k in teks_lower for k in ["lunas", "dibayar lunas", "sudah bayar", "sudah lunas", "udah lunas", "udah bayar", "bayar penuh"]):
    ...
    if not entitas["METODE_PEMBAYARAN"]:
        metode_list = ["tunai", "cash", "kontan", "transfer", "tf", "trf", "qris", "bca", "bri", "mandiri", "gopay", "dana", "ovo", "shopeepay"]
    ...
    greetings = [r"\bhallo\b", r"\bhalo\b", r"\bassalamualaikum\b", r"\bwuy\b", r"\bpagi\b", r"\bsiang\b", r"\bsore\b", r"\bmalam\b", r"\bp\b", r"\bbot\b"]
    ...
    if any(k in teks_lower for k in ["cek", "tampilkan", "tampilan", "berapa", "berpa", "brapaan", "siapa", "sapa", "lihat", "laporan", "laproan", "rekap", "dashboard", "dasbor"]):
        if any(k in teks_lower for k in ["harga", "barang", "produk"]):
            entitas["AKSI"] = "Cek Harga Barang"
            if generic_price_lookup_match and not (generic_price_lookup_match.group(2) or "").strip():
                entitas["BARANG"] = None
                entitas["NAMA"] = None
        else:
            entitas["AKSI"] = "Read Data"
        ...
        if entitas["AKSI"] == "Read Data":
            if any(k in teks_lower for k in ["nyicil", "cicil", "cicilan", "dp"]):
                entitas["STATUS"] = "Dicicil"
            elif any(k in teks_lower for k in ["hutang", "utang", "ngutang", "belum lunas", "tunggakan", "tagihan"]):
                entitas["STATUS"] = "Hutang"
                
            if any(k in teks_lower for k in ["total", "jumlah", "ttal", "akumulasi", "berpa", "berapa"]):
                if any(k in teks_lower for k in ["uang masuk", "pemasukan"]): entitas["KONTEKS_AGREGASI"] = "Uang Masuk"
                elif any(k in teks_lower for k in ["tunggakan", "tagihan", "hutang", "piutang", "belum lunas"]): entitas["KONTEKS_AGREGASI"] = "Total Tunggakan"
                elif "cicilan" in teks_lower: entitas["KONTEKS_AGREGASI"] = "Total Cicilan"
                else: entitas["KONTEKS_AGREGASI"] = "Total Transaksi"
            elif any(k in teks_lower for k in ["terbanyak", "paling banyak", "terbangak", "paking banyak", "paling gede", "plg gede"]):
                if any(k in teks_lower for k in ["pembeli", "nama", "orang"]):
                    ...
                    entitas["KONTEKS_AGREGASI"] = "Total Transaksi"
            elif any(k in teks_lower for k in ["siapa", "sapa", "siapakah"]) and \
                 any(k in teks_lower for k in ["tunggakan", "hutang", "belum lunas", "ngutang", "piutang"]):
                # Jika tanya "siapa yang ngutang", tampilkan daftar penunggak terbanyak/rekap debitur
                entitas["KONTEKS_AGREGASI"] = "Tunggakan Terbanyak"
                entitas["STATUS"] = "Hutang"
            elif any(k in teks_lower for k in ["siapa", "sapa", "siapakah"]) and \
                 any(k in teks_lower for k in ["lunas", "lunasin", "sudah bayar"]):
                entitas["KONTEKS_AGREGASI"] = "Summary Lunas"
                entitas["STATUS"] = "Lunas"
            elif any(k in teks_lower for k in ["tunggakan", "tagihan", "piutang", "hutang", "belum lunas"]):
                entitas["KONTEKS_AGREGASI"] = "Total Tunggakan"
                entitas["STATUS"] = "Hutang"
            ...
            if any(k in teks_lower for k in ["dashboard", "dasbor", "laporan harian", "rekap hari ini"]):
                entitas["KONTEKS_AGREGASI"] = "Dashboard Harian"
        ...
        if any(k in teks_lower for k in ["barang", "produk"]):
            entitas["AKSI"] = "Tambah Barang"
            if generic_master_create_match and not (generic_master_create_match.group(2) or "").strip():
                entitas["BARANG"] = None
                entitas["NAMA"] = None
                entitas["HARGA"] = None
                entitas["SATUAN"] = None
    elif any(k in teks_lower for k in ["hapus", "delete", "batalin", "apush"]):
        explicit_master_tail = (generic_master_delete_match.group(2) or "").strip() if generic_master_delete_match else ""
        if entitas.get("NAMA") and str(entitas["NAMA"]).strip().lower() in {"saya", "aku", "kami", "kita", "gue", "gua"}:
            entitas["NAMA"] = None
        is_transaksi_ctx = any(k in teks_lower for k in ["transaksi", "pesanan", "order", "orderan", "penjualan", "pembelian"]) or bool(entitas.get("TANGGAL")) or bool(entitas.get("NAMA"))
        is_master_phrase = bool(generic_master_delete_match) or (" master" in f" {teks_lower}") or (" daftar barang" in f" {teks_lower}") or (" list barang" in f" {teks_lower}")
        ...
        explicit_master_tail = (generic_master_edit_match.group(2) or "").strip() if generic_master_edit_match else ""
        is_transaksi_ctx = any(k in teks_lower for k in ["transaksi", "pesanan", "order", "orderan", "penjualan", "pembelian"]) or any(k in teks_lower for k in ["lunas", "hutang", "dicicil", "cicil", "status"]) or bool(entitas.get("TANGGAL")) or bool(entitas.get("NAMA"))
        has_price_number = bool(re.search(r"\b\d{3,}\b", teks_lower))
        is_master_phrase = ("harga" in teks_lower) or bool(generic_master_edit_match) or (bool(entitas.get("BARANG")) and has_price_number and not entitas.get("NAMA"))
        if is_master_phrase and not is_transaksi_ctx:
            entitas["AKSI"] = "Set Harga Barang"
    ...
    if entitas.get("AKSI") == "Read Data":
        has_transaksi_verb = any(k in teks_lower for k in ["ambil", "order", "pesan", "beli", "catat", "input"])
    ...
    if generic_status_update_match and not any(
        m for m in (generic_master_edit_match, generic_master_create_match, generic_master_delete_match, generic_price_lookup_match)
    ):
        entitas["AKSI"] = "Update Status"
...
def tentukan_intent(entitas, teks_lower):
    ...
    if ds_intent and ds_score >= 85:
        allow = True
        if ds_intent in {"Read_Analitik_Penjualan", "Read_Analitik_Hutang", "Read_Transaksi_Spesifik"}:
            allow = aksi == "Read Data"
        elif ds_intent == "Chit_Chat":
            allow = aksi == "Chit Chat"
        elif ds_intent == "CRUD_Barang":
            allow = aksi in {"Tambah Barang", "Cek Harga Barang", "Set Harga Barang", "Hapus Barang"}
