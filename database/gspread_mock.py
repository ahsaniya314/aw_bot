...
class SupabaseWorksheet:
    ...
    def get_all_values(self):
        ...
        for idx, row in enumerate(data):
            ...
            if self.table_name == "transaksi":
                ...
                result.append(
                    [str(row.get("id")), row.get("nama_metode", ""), row.get("kata_kunci", "")]
                )
    ...
    def update_cells(self, cell_list, value_input_option="USER_ENTERED"):
        ...
        for row, cols in row_data.items():
            ...
            if data:
                self._get_supabase().table(self.table_name).update(data).eq(
                    "id", supabase_id
                ).execute()
    ...
    def row_values(self, row_idx):
        ...
        supabase_id = self.row_mapping[row_idx]
        res = (
            self._get_supabase().table(self.table_name).select("*").eq("id", supabase_id).execute()
        )
        if res.data:
            row = res.data[0]
            if self.table_name == "transaksi":
